...
(function($) {
    ...
    function getModule() {
        var m = window.location.search.match(/[?&]module=([^&]+)/);
        return m ? decodeURIComponent(m[1]) : ($('input[name="module"]').val() || '');
    }

    function getView() {
        var v = window.location.search.match(/[?&]view=([^&]+)/);
        return v ? decodeURIComponent(v[1]) : ($('input[name="view"], input#view').val() || 'List');
    }
